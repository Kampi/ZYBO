...
/** @file VideoPlayer/Video/VideoTiming/ClockingWizard/ClockingWizard.h
...
static u32 ClockingWizard_WaitForLock(ClockingWizard_t* Device, const u32 Timeout)
{
	...
	while(!(Xil_In32(Device->BaseAddr + OFFSET_CLOCKINGWIZARD_STATUS) & (0x01 << CLOCKINGWIZARD_BIT_LOCKED)))
	{
	...
	Device->isLocked = Xil_In32(Device->BaseAddr + OFFSET_CLOCKINGWIZARD_STATUS) & (0x01 << CLOCKINGWIZARD_BIT_LOCKED) >> CLOCKINGWIZARD_BIT_LOCKED;
...
u32 ClockingWizard_GetOutput(ClockingWizard_t* Device, ClockOut_t* Output)
{
...
u32 ClockingWizard_SetOutput(ClockingWizard_t* Device, ClockOut_t* Output)
{
	u32 BaseAddress = 0x00;
	...
	Xil_Out32(Device->BaseAddr + BaseAddress, (((u32)Output->FRAC_Divide << 0x08)) | Output->DIVIDE);
	Xil_Out32(Device->BaseAddr + BaseAddress + 0x04, Output->PHASE);
	Xil_Out32(Device->BaseAddr + BaseAddress + 0x08, Output->DUTY);
